...
- **Dashboard (top-right)** shows the live regime — `CHOP — STAND ASIDE` or
  `TRADEABLE` — plus the current Choppiness Index and ADX readings against
  their thresholds.
...
| ADX | below threshold | < 22 |

A zone is only drawn after **5 consecutive choppy bars** (configurable), and
only closes after **3 consecutive trending bars**, so single-bar fakeouts
don't flicker zones on and off.
